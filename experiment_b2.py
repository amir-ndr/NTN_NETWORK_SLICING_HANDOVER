#!/usr/bin/env python3
"""
B2 experiment: random poly-chain dispatcher. Runs N Xn handovers via gnb2_b2.yaml
(dispatcher enabled), writes latencies to xn_ho_latency_b2.csv.

Prerequisites — run in separate terminals BEFORE this script:
  sudo ./build/nr-gnb -c config/gnb1.yaml
  sudo ./build/nr-gnb -c config/gnb2_b2.yaml     ← uses B2 config (dispatcher enabled)
  python3 dispatcher.py                            ← B2 random dispatcher

Run:
  sudo python3 experiment_b2.py [N]   (default N=50)
"""

import subprocess, time, sys, os, shutil, threading, queue, re

if os.geteuid() != 0:
    sys.exit("Run with sudo: sudo python3 experiment_b2.py [N]")
...
for i in range(N):
    ...
    # B2: dispatcher adds chain latency (up to ~60ms); give it extra time
    time.sleep(4)
